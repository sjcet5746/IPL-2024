...
if selection == "Add Student":
    ...
    # Bar graph for total marks by branch
    if st.button("Show Bar Graph by Branch"):
        branch_totals = {}
        ...
        for roll, info in student_data.items():
            ...
            if branch in branch_totals:
                branch_totals[branch] += total_marks
            else:
                branch_totals[branch] = total_marks

        # Plotting the bar graph
        plt.figure(figsize=(10, 5))
        plt.bar(branch_totals.keys(), branch_totals.values(), color='skyblue')
        plt.xlabel('Branches')
        plt.ylabel('Total Marks')
        plt.title('Total Marks by Branch')
        st.pyplot(plt)
